...
public class Inventory : MonoBehaviour
{
    ...
    public void EquipMove(Move move){    
        bool styleMatchFound = false;
        MoveSet moveSetToUpdate = new MoveSet();
        foreach(MoveSet moveSet in activeMoves){
            if (moveSet.name == move.style){
                styleMatchFound = true;
                moveSetToUpdate = moveSet;
                //Debug.Log("Style match founnd for " + moveSetToUpdate.moves[0].name);
            }
        }

        Debug.Log("Stylematch found = " + styleMatchFound);

        if(styleMatchFound){
            //Debug.Log("Equipping move " + move.name + " to existing style " + moveSetToUpdate.name);
            if(isMoveEquipped(move)){
                Debug.Log("!Move Already Equipped!");
            }else{
                moveSetToUpdate.moves = AddMoveToArray(moveSetToUpdate.moves,move,false);
            }
        }else{
            //Debug.Log("Equipping move " + move.name + " to new style " + moveSetToUpdate.name);   
            MoveSet newMoveSet = new MoveSet();
            newMoveSet.moves = new Move[1];
            newMoveSet.moves[0] = move;
            newMoveSet.name = move.style;
            //Debug.Log("Equipping move " + move.name + " to style " + newMoveSet.name + " with length " + newMoveSet.moves.Length);
            activeMoves = AddMoveSetToArray(activeMoves,newMoveSet); 
            Move[] movesArrayToUpdate = newMoveSet.moves;
            Debug.Log("Equipping move " + movesArrayToUpdate.Length); 
            //movesArrayToUpdate = AddMoveToArray(movesArrayToUpdate,move,true);
        }
        GenerateEquippedStylesList();
        if(inventoryDisplay == null){
            Debug.LogError("Inventory Display not found!!");
        }else{
            inventoryDisplay.PopulateEquippedMoveList(inventoryDisplay.menuCursor.y);
        }
    }
